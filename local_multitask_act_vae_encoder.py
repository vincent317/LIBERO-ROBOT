#!/usr/bin/env python3
"""Local ACT variant with baseline task-id conditioning plus task-id in the VAE encoder."""
...
@PreTrainedConfig.register_subclass("act_task_id_vae_encoder")
@dataclass
class ACTTaskIDVAEEncoderConfig(ACTConfig):
    num_task_ids: int = 1
...
class TaskConditionedACTVAEEncoder(ACT):
    def __init__(self, config: ACTTaskIDVAEEncoderConfig):
...
class ACTTaskIDVAEEncoderPolicy(ACTPolicy):
    config_class = ACTTaskIDVAEEncoderConfig
    name = "act_task_id_vae_encoder"

    def __init__(self, config: ACTTaskIDVAEEncoderConfig, **kwargs):
        PreTrainedPolicy.__init__(self, config)
        config.validate_features()
        self.config = config
        self.model = TaskConditionedACTVAEEncoder(config)
